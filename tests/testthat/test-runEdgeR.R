...
test_that("runEdgeR works with multiple comparisons", {
    ...
    tmp1 <- tempfile()
    alone1 <- runEdgeR(se, group="group", comparisons=c("C", "A"), subset.group=FALSE, output.dir=tmp1)
    expect_identical(out$results$foo, alone1$results[[1]])

    tmp2 <- tempfile()
    alone2 <- runEdgeR(se, group="group", comparisons=c("B", "D"), subset.group=FALSE, output.dir=tmp2)
    expect_identical(out$results$bar, alone2$results[[1]])
})

test_that("runEdgeR works with log-fold changes", {
    tmp <- tempfile()
    out <- runEdgeR(se, group="group", comparisons=c("C", "A"), lfc.threshold=0.5, output.dir=tmp)
...
test_that("runEdgeR works with more annotations", {
    ...
    out <- runEdgeR(se, group="group", comparisons=c("C", "A"), row.data="symbol", output.dir=tmp)
...
test_that("runEdgeR works with some of the other edgeR-related options", {
    tmp <- tempfile()
    out <- runEdgeR(se, group="group", comparisons=c("C", "A"), robust=FALSE, trend=FALSE, output.dir=tmp)
