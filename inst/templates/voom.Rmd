...
```{r test-<%= CONTRAST_CHUNK_NAME %>}
fit2 <- limma::contrasts.fit(fit, con)
fit2 <- limma::treat(fit2, lfc=<%= LFC_THRESHOLD %><%= EXTRA_EB_ARGS %>)
```
:END
...
```{r test-<%= CONTRAST_CHUNK_NAME %>}
fit2 <- limma::contrasts.fit(fit, con)
fit2 <- limma::eBayes(fit2<%= EXTRA_EB_ARGS %>)
```
:END
...
```{r}
colnames(deres)[colnames(deres) == "P.Value"] <- "PValue"
colnames(deres)[colnames(deres) == "adj.P.Val"] <- "FDR"

```{r, eval=is.null(dim(con)), echo=is.null(dim(con))}
colnames(deres)[colnames(deres) == "logFC"] <- "LogFC"
```

```{r, eval=!is.null(dim(con)), echo=!is.null(dim(con))}
expected <- make.names(colnames(con))
m <- match(expected, colnames(deres))
stopifnot(!anyNA(m), !anyDuplicated(m))
colnames(deres)[m] <- paste0("LogFC.", colnames(con))
```

We store the results in our output list.

```{r}
all.results[[<%= CONTRAST_NAME_DEPARSED %>]] <- df
```
...
```{r <%= SAVING_CHUNK_NAME %>}
de.meta <- list(
    title=<%= DEPARSED_CONTRAST_NAME %>,
    authors=<%= AUTHOR %>,
    type="differential_gene_expression",
    differential_gene_expression=list(
:BEGIN diff-result-metadata
:END
:BEGIN subset-result-metadata
:END
        method="voom-limma"
    )
)
:BEGIN merge-metadata-diff

de.meta <- c(common.meta, de.meta)
de.meta <- de.meta[!duplicated(names(de.meta))]
:END
